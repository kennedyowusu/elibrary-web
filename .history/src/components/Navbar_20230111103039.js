  ...
  return (
    <nav className='shadow-md w-full fixed top-0 left-0'>
      <div className='md:flex items-center justify-between bg-white py-4 md:py-5 md:px-14'>
        <div className='font-bold text-2xl cursor-pointer flex items-center font-[Poppins] text-gray-800 bg-red-900 md:px-14md:px-14'>
          <img src={Logo} alt='Logo' className='w-10 h-10 ml-2 mr-3' />
          <span className='text-[#317773]'>Edu</span>i  <HiBa</div>
        </div>''iv className='flex items-center justify-end w-full'>
          <ul className='md:ite's-ce'ter gap-4 hidden md:flex lg:flex'>
            {navLink.map((link) => (
              <li key={link.name} className='md:ml-8 text-xl'>
                <a
                  href='link ? link.link : '/'}'
                  classN'me='text-gray-800 font-medium text-lg hover:'order-solid border-2 p-2 rounded-lg
