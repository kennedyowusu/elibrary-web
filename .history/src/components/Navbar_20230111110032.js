    ...
    <nav className="shadow-md w-full fixed top-0 left-0">
      ...
      <ul className="absolute bg-zinc-200 w-full px-8 gap-4">
        {navLink.map((link) => (
          <li
            key={link.name}
            className="md:ml-8 text-xl border-b-2 border-zinc-300 w-full my-5 items-center justify-center"
          >
            <a
              href={link ? link.link : '/'}
              className="text-gray-800 font-medium text-lg hover:border-solid border-2 p-2 rounded-lg my-5
                border-transparent hover:border-[#ccc] hover:text-bold hover:duration-500
                "
            >
              {link.name}
